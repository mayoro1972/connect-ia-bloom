alter table public.contact_requests
  add column if not exists prospect_username text,
  add column if not exists prospect_password_hash text,
  add column if not exists prospect_portal_status text not null default 'pending',
  add column if not exists audit_invite_token text,
  add column if not exists audit_invite_expires_at timestamp with time zone,
  add column if not exists last_portal_login_at timestamp with time zone;

alter table public.contact_requests
  drop constraint if exists contact_requests_prospect_portal_status_check;

alter table public.contact_requests
  add constraint contact_requests_prospect_portal_status_check
  check (prospect_portal_status in ('pending', 'active', 'disabled'));

create unique index if not exists contact_requests_prospect_username_idx
  on public.contact_requests (lower(prospect_username))
  where prospect_username is not null;

create index if not exists contact_requests_audit_portal_lookup_idx
  on public.contact_requests (request_intent, lower(email), lower(prospect_username));
...
create table if not exists public.prospect_email_delivery_logs (
  ...
  constraint prospect_email_delivery_logs_status_check check (status in ('queued', 'sent', 'failed', 'skipped')),
  constraint prospect_email_delivery_logs_delivery_type_check check (
    delivery_type in ('internal_notification', 'acknowledgement', 'audit_explainer', 'qualified_response', 'audit_form_access')
  )
);

create index if not exists prospect_email_delivery_logs_contact_request_idx
  on public.prospect_email_delivery_logs (contact_request_id, created_at desc);

create index if not exists prospect_email_delivery_logs_recipient_idx
  on public.prospect_email_delivery_logs (recipient_email, created_at desc);
...
revoke all on table public.prospect_email_delivery_logs from anon, authenticated;
