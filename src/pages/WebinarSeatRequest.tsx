...
const copy = {
  fr: {
    badge: "Demande de place",
    title: "Réserver ma place au webinaire",
    subtitle:
      "Complétez le formulaire pour réserver votre place. Notre équipe vous contactera sous 48h avec les modalités d'accès, les conditions de participation et le tarif applicable.",
    ...
    consent:
      "J'accepte que TransferAI Africa traite mes données pour étudier ma demande de place et me recontacter avec les modalités.",
    submit: "Envoyer ma demande de place",
    submitting: "Envoi…",
    success: "Demande envoyée",
    successText:
      "Merci ! Notre équipe vous contactera sous 48h avec les modalités d'accès et les conditions de participation à ce webinaire.",
    backHome: "Retour à l'accueil",
    error: "Une erreur est survenue. Merci de réessayer.",
    other: "Autre",
    none: "— Sélectionner —",
    notice:
      "Ce webinaire est un format premium à accès limité. Les conditions de participation et le tarif vous seront communiqués par notre équipe après réception de votre demande.",
    plannedDate: (date: string) => `Dates prévisionnelles : ${date} — heures à préciser (confirmation par email)`,
  },
  en: {
    badge: "Seat request",
    title: "Reserve my webinar seat",
    subtitle:
      "Fill out this form to request a seat. Our team will get back to you within 48 hours with the access details, participation conditions and applicable fee.",
    ...
    consent:
      "I agree that TransferAI Africa processes my data to review my seat request and contact me with the participation details.",
    submit: "Send my seat request",
    submitting: "Sending…",
    success: "Request sent",
    successText:
      "Thank you! Our team will reach out within 48 hours with the access details and participation conditions for this webinar.",
    backHome: "Back to home",
    error: "An error occurred. Please try again.",
    other: "Other",
    none: "— Select —",
    notice:
      "This webinar is a premium, limited-seat format. Participation conditions and fee will be shared by our team after we receive your request.",
...
const WebinarSeatRequest = () => {
  ...
  const initialMotivation = initialTopic
    ? lang === "fr"
      ? `Je souhaite réserver une place au webinaire « ${initialTopic} »${initialDate ? ` du ${initialDate}` : ""}.`
      : `I would like to reserve a seat for the webinar "${initialTopic}"${initialDate ? ` on ${initialDate}` : ""}.`
    : "";
  ...
  const plannedDate = useMemo(() => {
    return lang === "fr"
      ? "entre le 17 juillet 2026 et le 19 juillet 2026"
      : "between July 17, 2026 and July 19, 2026";
  }, [lang]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const motivationWithFlag = `[DEMANDE DE PLACE — WEBINAIRE PAYANT]\nSujet : ${initialTopic || "(non précisé)"}\n\n${form.motivation || ""}`.trim();
      ...
      const { error: rpcError } = await supabase.rpc("submit_webinar_registration", {
        ...
        formation_title_input: formationLabel || initialTopic || null,
        ...
        motivation_input: motivationWithFlag,
          ...
          <div className="container mx-auto max-w-3xl px-4 lg:px-8">
            {done ? (
              ...
              <form onSubmit={submit} className="space-y-8 rounded-3xl border border-border bg-card p-8">
                {initialTopic && (
                  <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-card-foreground">
                    <p className="flex items-center gap-2 font-semibold">
                      <BadgeCheck size={16} className="text-primary" /> {initialTopic}
                    </p>
                  </div>
                )}

                <div className="rounded-2xl bg-accent/10 p-4 text-sm text-card-foreground">
...
export default WebinarSeatRequest;
