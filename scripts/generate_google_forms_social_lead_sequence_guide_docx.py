...
H1_COLOR = RGBColor(0x1B, 0x43, 0x6B)
...
def configure_document(document: Document) -> None:
    ...
    h1.paragraph_format.line_spacing = 1.12
    ...
    h2.paragraph_format.line_spacing = 1.12
    ...
    footer_run = footer.add_run(
        "TransferAI - Guide Google Forms Social Lead Sequence - 15 juin 2026"
    )
    set_run_font(footer_run, size=8.5, color=MUTED_COLOR)
...
def add_title_block(document: Document, title: str) -> None:
    ...
    run = p.add_run(title)
    set_run_font(run, size=23, bold=True, color=RGBColor(0x0B, 0x25, 0x45))
    set_paragraph_spacing(p, before=0, after=4, line=1.03)

    subtitle = document.add_paragraph()
    srun = subtitle.add_run(
        "Guide opératoire, variables, procédure de test et dépannage pour le workflow n8n Google Forms vers CRM et séquence email."
    )
    set_run_font(srun, size=10.5, color=MUTED_COLOR)
    set_paragraph_spacing(subtitle, before=0, after=12, line=1.12)
...
def add_inline_runs(paragraph, text: str) -> None:
    ...
    for part in parts:
        ...
        if part.startswith("`") and part.endswith("`"):
            ...
            set_run_font(run, name="Consolas", size=9.2, color=RGBColor(0x11, 0x18, 0x27))
...
def add_number(document: Document, number: str, text: str) -> None:
    p = document.add_paragraph(style="Normal")
...
def add_code_block(document: Document, lines: list[str]) -> None:
    for idx, line in enumerate(lines):
        ...
        set_paragraph_spacing(
            p,
            before=4 if idx == 0 else 0,
            after=0 if idx < len(lines) - 1 else 8,
            line=1.08,
        )
...
def export_markdown(md_path: Path, docx_path: Path) -> None:
    ...
    for raw in lines:
        ...
        if line.startswith("# "):
            if not first_heading_used:
                ...
                set_paragraph_spacing(p, before=18, after=10, line=1.12)
            continue
        ...
        if line.startswith("### "):
            ...
            set_paragraph_spacing(p, before=14, after=7, line=1.12)
            continue
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            "Usage: generate_google_forms_social_lead_sequence_guide_docx.py <input.md> <output.docx>"
        )
